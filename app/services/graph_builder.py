...
class GraphBuilder:
    ...
    def __init__(self, 
        ...
        try:
            ...
            logger.info("Neo4j connection established", uri=neo4j_uri, user=neo4j_user)
        except Exception as e:
            logger.error("Failed to connect to Neo4j", error=str(e), uri=neo4j_uri)
            self.driver = None
    ...
    def _calculate_edge_weight(self, chunk1: SemanticChunk, chunk2: SemanticChunk) -> Tuple[float, Dict[str, float]]:
        ...
        relationship_scores[RelationshipType.CAUSAL] = position_score * semantic_sim  # Causal likely if close and similar
